...
export default function Case({title,number}){
    return(
        <div className="flex flex-col min-w-[33%] px-4 py-12">
            <div className="w-full bg-white h-[1px]"></div>
            <div className="flex items-center pt-2">
                <div className="w-4 h-4 rounded-full bg-white"></div>
                <div className="text-white pl-2 text-[16px] leading-6"><div className={orb7.className}>{title}</div></div>
            </div>
            <div className="text-[42px] leading-[50px] uppercase text-white"><div className={outfit7.className}>{number}</div></div>
        </div>
    )
}